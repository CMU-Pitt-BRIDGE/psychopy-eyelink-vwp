...
#
# Notes:
#   1) See "Walkthrough_Building EyeLink-Compatible PsychoPy Experiments with VWP.docx"
...
from psychopy import (core, data, event, gui, logging, monitors, os, sound,
                      visual)
...
Wait = (u'Pulsa la barra espaciadora.')
fixation = ('+')
...
def run_trial(trial_pars, trial_index):
    ...
    play_img = visual.ImageStim(win,
                           image=os.path.join('images', 'center_play_button.png'),
                           size=[150,150], pos=center_coordinates)

    # load target image
    target_img = visual.ImageStim(win,
                           image=os.path.join('images', target_picture),
                           size=stimsize, pos=target_pos)

    # load comp_1 image
    comp_1_img = visual.ImageStim(win,
                               image=os.path.join('images', comp_1_picture),
                               size=stimsize, pos=comp_1_pos)

    # load comp_2 image
    comp_2_img = visual.ImageStim(win,
                               image=os.path.join('images', comp_2_picture),
                               size=stimsize, pos=comp_2_pos)

    # load comp_3 image
    comp_3_img = visual.ImageStim(win,
                               image=os.path.join('images', comp_3_picture),
                               size=stimsize, pos=comp_3_pos)
    ...
    displayComponents = [target_img, comp_1_img, comp_2_img, comp_3_img, play_img, mouse]
    ...
    # keep track of which components have finished
    play_soundComponents = [trial_audio, target_img, comp_1_img, comp_2_img, comp_3_img, mouse]
